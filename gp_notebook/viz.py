...
def notebook_hero_html() -> str:
    """Return the opening hero banner with paper metadata and garden-path fork SVG."""
    return """
<style>
  ...
  <div class="gp-hero__grid">
    ...
    <div style="background:rgba(255,255,255,0.75);border-radius:10px;padding:14px;
      ...
      <svg viewBox="0 0 280 140" width="100%" role="img"
        ...
        <text x="8" y="18" font-size="11" fill="#64748b">Ambiguous noun</text>
...
def interpolate_intervention(
    ...
    *,
    ...
    """Blend baseline and syntactic intervention stats from slider amplitudes."""
    if use_random:
        return {
            "mean_p_gp": baseline["mean_p_gp"],
            "mean_p_non_gp": baseline["mean_p_non_gp"],
            "mean_diff": baseline["mean_diff"],
            "source": "precomputed-random",
        }
    strength = min(1.0, (abs(subject_amp) + abs(object_amp) + abs(clause_amp)) / 6.0)
    if strength < 0.05:
        return {**baseline, "source": "precomputed-baseline"}
    alpha = strength
    return {
        "mean_p_gp": baseline["mean_p_gp"] * (1 - alpha) + syntactic["mean_p_gp"] * alpha,
        "mean_p_non_gp": baseline["mean_p_non_gp"] * (1 - alpha) + syntactic["mean_p_non_gp"] * alpha,
        "mean_diff": baseline["mean_diff"] * (1 - alpha) + syntactic["mean_diff"] * alpha,
        "source": "precomputed-blend",
    }
...
def circuit_svg(category_counts: pd.DataFrame) -> str:
    ...
    for row_idx, layer in enumerate(layers):
        y = 30 + row_idx * 70
        chunks.append(f'<text x="10" y="{y}" font-size="14" fill="#555">Layer {layer}</text>')
...
def probe_figure5_plot(probe_data: dict[str, list[dict[str, float | int | str]]], condition: str) -> go.Figure:
    """Line chart of structural probe action probabilities (Figure 5)."""
    ...
    fig.update_layout(
        title=f"Structural probe actions ({condition}) — Figure 5",
        xaxis_title="Layer",
