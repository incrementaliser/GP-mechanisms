...
class RuntimeState:
    ...
    hf_tokenizer: AutoTokenizer | None = None
    nnsight_model: LanguageModel | None = None
...
def get_hf_model() -> tuple[AutoModelForCausalLM, AutoTokenizer]:
...
def get_nnsight_bundle() -> tuple[LanguageModel, dict[str, Any]]:
    """Load nnsight model and SAE dictionaries once for intervention cells."""
    from gp_notebook.interventions import load_dictionaries, saes_available
...
def runtime_status_line() -> str:
    ...
    from gp_notebook.interventions import saes_available
